...
const Homescreen = () => {
  ...
  const connectWallet = async () => {
    try {
      ...
      const provider =
        (await new ethers.providers.Web3Provider(instance)) || "";
  ...
  const mintButton = async (mintValue) => {
    if (!account) {
      connectWallet();
      return;
    }
    ...
    try {
      ...
      console.log(error);
      toast.error("Whoops , something went wrong! or Insufficient Funds", {
        id: notification,
      });
    }
  };
